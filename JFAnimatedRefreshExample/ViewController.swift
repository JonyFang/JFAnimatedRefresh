...
class ViewController: UIViewController {
    ...
    deinit {
        tableView.jf_removePullToRefresh()
    }
    ...
    fileprivate func setupViews() {
        ...
        let loadingView = JFAnimatedRefreshLoadingCircle()
        loadingView.tintColor = UIColor(red: 78/255.0, green: 221/255.0, blue: 200/255.0, alpha: 1.0)
        
        tableView.jf_addPullToRefreshWithHandler({ [weak self] () -> Void in
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(1.5 * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC), execute: {
                self?.tableView.jf_stopLoading()
            })
            }, loadingView: loadingView)
        tableView.jf_setPullToRefreshFillColor(UIColor(red: 57/255.0, green: 67/255.0, blue: 89/255.0, alpha: 1.0))
        tableView.jf_setPullToRefreshBackgroundColor(tableView.backgroundColor!)
    }
}
